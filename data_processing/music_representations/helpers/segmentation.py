...
def build_segments(
    ...
    for piece in pieces.iter_rows(named=True):
        ...
        piece_notes = notes.filter(pl.col("piece_id") == piece_id)
        segments.extend(build_piece_segments(piece, piece_notes, config, hop_ticks))
    return pl.DataFrame(segments)


def build_piece_segments(
    piece: dict[str, object],
    piece_notes: pl.DataFrame,
        ...
        return [
            {
                ...
                "start_tick": 0,
                "end_tick": piece["end_tick"],
                "configuration_hash": config.config_hash(),
            }
        ]

    segment_ticks = config.segmentation.segment_ticks
    resolved_hop_ticks = hop_ticks or config.segmentation.hop_ticks or segment_ticks
    if segment_ticks is None:
        raise ValueError("Segmentation enabled but segment_ticks is missing")
    assert resolved_hop_ticks is not None
    resolved_hop_ticks = int(resolved_hop_ticks)
    ...
    while current < end_tick or (current == 0 and end_tick == 0):
        ...
        if in_segment.height >= config.segmentation.min_notes:
            stable_key = f"{piece_id}:{current}:{segment_end}:{config.config_hash()}"
            segments.append(
                {
                    "piece_id": piece_id,
                    "segment_id": sha256(stable_key.encode("utf-8")).hexdigest()[:16],
                    "maestro_split": piece["maestro_split"],
                    "start_tick": current,
